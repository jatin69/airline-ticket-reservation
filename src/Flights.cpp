...
Fnode* Flights::findFlight(char* _source, char* _dest) const {
  ...
  if (strcasecmp(tail->getSource(), _source) == 0 &&
      strcasecmp(tail->getDestination(), _dest) == 0)
    return tail;

  Fnode* temp = head;
  while (temp->next) {
    if (strcasecmp(temp->getSource(), _source) == 0 &&
        strcasecmp(temp->getDestination(), _dest) == 0)
      return temp;
  ...
  try {
    ...
    if (temp->isFull())
      ...
      for (int i = 0; i < noOfTkts; i++) {
        ...
        cout << "\nYour ticket no. is: " << temp->getFcode() << ticketNo
             << endl;
      }
    }
    return true;
